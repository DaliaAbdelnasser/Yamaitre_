    ...
    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
        ...
        <thead>
            ...
            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                ...
                <th class="min-w-125px">تاريخ نشر المهمة 	</th>
        ...
        @foreach ($others_inprogress as $task)
            <!--begin::Table row-->
            <tr>
                <!--begin::User=-->
                    ...
                    <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                        <a href="">
                            <div class="symbol-label fs-3 bg-light-danger text-danger">
                                ...
                                <img src="{{ asset('uploads/' . $task->user->first()->userable->profile_image) }}" alt="{{ $task->user->first()->first_name ?? ''}}" class="w-100">
                                @else
                                {{ strtoupper(substr($task->user->first()->first_name ?? '', 0, 1)) }}
                                @endif
                            </div>
                        </a>
                    </div>
                    <span class="text-gray-800 text-hover-primary mb-1">{{ $task->user->first()->first_name ?? ''}}</span>
                    <div class="row">
