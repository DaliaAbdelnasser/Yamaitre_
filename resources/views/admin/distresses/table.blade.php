...
<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
	...
	<thead>
		...
		<tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
			<th class="min-w-125px">نوع الإستغاثة</th>
	...
	@foreach ($distresses as $dist)
        <!--begin::Table row-->
        <tr>
            <td >
                <!--begin:: Avatar -->
                <a class="text-gray-800 text-hover-primary mb-1">{{ $dist->type }}</a>
                    <span>{{ $dist->created_at }}</span>
				
            </td>
            <td>{{ $dist->governorate ?? ''}}</td>
            <td> {{ $dist->description  ?? ''}}</td>
            <td> {{ $dist->users->first()->first_name ?? ''}}</td>
            <td> {{ $dist->users->first()->phone ?? ''}}</td>
            <td> {{ $dist->users->first()->email ?? ''}}</td>
            ...
            <td class="text-end">
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                    ...
                    <div class="menu-item px-3">
                        <a href="" class="menu-link px-3" data-kt-users-table-filter="delete_row" data-bs-toggle="modal" data-bs-target="#delete_item-{{$dist->id}}">حذف</a>
                    </div>
                    <!--end::Menu item-->
                </div>
                <!--end::Menu-->
        ...
        <div class="modal fade" id="delete_item-{{$dist->id}}" tabindex="-1" role="dialog" aria-labelledby="delete_item-{{$dist->id}}" aria-hidden="true">
            <!--begin::Modal dialog-->
            <div class="modal-dialog modal-dialog-centered mw-650px">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                        ...
                        <div class="swal2-html-container my-15" id="" style="display: block;">هل أنت متأكد أنك تريد حذف هذه الاستغاثة ؟</div>
                        <div class="swal2-actions" style="display: flex;">
                            <div class="swal2-loader"></div>
                            {!! Form::model($dist, ['route' => ['admin.distresses.destroy', $dist->id], 'method' => 'delete']) !!}
...
@include('partials._pagination', ['records' => $distresses])
